...
\begin{longtable}{|c|p{5cm}|p{8cm}|}
	...
	1. & Rejestracja użytkownika & System umożliwia utworzenie konta użytkownika. \\ \hline
	2. & Logowanie do systemu & System weryfikuje dane użytkownika i~przyznaje ciasteczko umożliwiające dostęp do aplikacji. \\ \hline
	3. & Wylogowanie & System usuwa ciasteczko użytkownika. \\ \hline
	...
	6. & Historia transakcji & Aplikacja wyświetla listę transakcji. \\ \hline
...
Jednym z~kluczowych etapów w~procesie tworzenia oprogramowanie jest wizualizacja systemu \cite{UML}. Język UML (ang. \textit{Unified Modeling Language}), czyli ujednolicony język modelowania nie jest językiem programowani, chociaż może uprościć proces tworzenia aplikacji, generując kod na podstawie diagramów. UML definiuje dwie podstawowe składowe: notację elementów na diagramach oraz ich semantykę \cite{UML1}.
\subsection*{Tworzenie diagramów}
Diagramy UML można utworzyć graficznie, na przykład przez stronę draw.io, bądź tekstowo odpowiednim językiem takim jak PlantUML lub mermaid. Można też skorzystać z~narzędzi takich jak enterprise architect, które posiada dodatkowe opcje, takie jak tworzenie kodu programu na bazie diagramów. 
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.87\textwidth]{images/PlantUML_useCase.png}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.87\textwidth]{images/MermaidSeqDiag}
	\caption{Diagram sekwencji - Nowa transakcja portfela}
	\label{fig:Seq1}
\end{figure}
\section{Diagram klas}
Przedstawia strukturę klas oraz ich relacje. Relacje między klasami opisują w~jaki sposób klasy są ze sobą związane. Najbardziej podstawowa relacja to zależność. Jest to najsłabszy rodzaj relacji, który występuje w momencie, gdy zmiany jednego obiektu mogą powodować zmiany w drugim obiekcie. 
Relacje możemy również podzielić na te związane z rozszerzeniem oraz interferencją klas. Do relacji związanych z~rozszerzeniem klas zalicza się implementacja (ang. \textit{implementation}) oraz dziedziczenie (ang. \textit{inheritance}), natomiast do relacji związanych z interferencją klas zaliczamy asocjację (ang. \textit{association}), agregację (ang. \textit{aggregation}) oraz kompozycję (ang. \textit{composition}). Każda relacja jest oznaczona w określony sposób \cite{UMLClass}.
\begin{figure}[H]
	\centering
	\includegraphics[height=0.9\textheight, keepaspectratio]{images/diagramKlas}
