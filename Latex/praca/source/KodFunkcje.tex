...
Warstwa serwerowa odpowiada ze przetwarzanie żądań od warstwy prezentacji oraz za komunikację z bazą danych. Elementy z których jest skonstruowana to m.in. kontrolery, repozytoria, czy pliki konfiguracyjne. Została zrealizowana w oparciu o framework Spring Boot.
...
\end{lstlisting}
Aby wykonać wiele testów dla jednej metody należy skorzystać z testów parametryzowanych, które są opisane adnotacją \texttt{@ParametrizedTest}. Pozwalają one określić zbiór na przykład podanie metody, która zwraca dane \texttt{@MethodSource("nazwa")}. Deklaracja testu dla wcześniej określonych wygląda następująco:
\begin{lstlisting}[language={Java}, caption={Deklaracja testu}, label={lst:arg-stream}]
@ParameterizedTest
...
Test ten ma określać, czy dodanie nowej transakcji się powiodło. Wszystkie mocki, które mamy należy nauczyć zachowań (ang. \textit{stubbing}), czyli wytłumaczyć jak mają reagować na metody.

\begin{lstlisting}[language={Java}, caption={Stubbing}, label={lst:arg-stream}]
...
Po procesie uczenia wywołujemy funkcję, która korzysta z wcześniej nauczonych metod oraz weryfikujemy, czy funkcja wykonała swoje zadanie. Odbywa się to za pomocą asercji oraz metod verify. Asercje polega na potwierdzeniu jakiegoś warunku, a verify sprawdza czy dane metoda była wywołana oraz ile razy ją wywołano (domyślnie jeden).

\begin{lstlisting}[language={Java}, caption={Weryfikacja}, label={lst:arg-stream}]
...
Ważnym aspektem testów jest również sprawdzanie, czy kontroler działa poprawnie. Udostępnia on endpointy, oraz przeprowadza walidację, więc sprawdzenie czy endpoint działa oraz czy podanie błędych danych jest obsłużone może być sprawdzone tylko na poziomie kontrolera. 
...
Funkcja rejestracji przyjmuje jako argument przygotowany model danych do autoryzacji, który posiada poprawnie uzupełnione ciało (String username, String passwrod). Jeżeli podane dane są poprawne, zostaje odpalona metoda w serwisie odpowiedzialna za rejestrację. Tworzy nowego użytkownika oraz szyfruje jego hasło.
...
\begin{lstlisting}[language={Java}, caption={Wyświetlanie transakcji od do}, label={lst:java-register}]
// Metoda w serwisie
...
\begin{lstlisting}[language={Java}, caption={Edytowanie transakcji w portfelu}, label={lst:java-delete}]
// Metoda w serwisie
...
Komunikacja z backendem odbywa się dzięki bibliotece axios, która wysyła żądania do serwera. W pliku axios.ts zawarta jest configuracja zapytań. Opisuje jaki adres ma wykorzystywać oraz czy wysyłać uwierzytelnienie (ang. \textit{Credentials}).
...
Stany są jednym z kluczowych funkcji, które wykorzystuje się w aplikacjach opartych o React. Wykorzystują funkcję haków (ang. \textit{hook}), które pozwalają używać stanów bez konieczności posiadania klasy. Do zarządzania stanami możemy użyć funkcji useState lub useEffect. Funkcja useState pozwala nam ustawić jakiś status podczas działania strony. Użytkownik może wybrać datę, następuje zmiana wartości pól date. Użycie efektu pozwala wykonać jakąś funkcję lub działanie i wpłynąć na to jaki status zostanie ustawiony. Pozwala również określić, od czego dany efekt jest zależny, w takim wypadku przy zmianie wartości zostanie ponownie wykonany.
